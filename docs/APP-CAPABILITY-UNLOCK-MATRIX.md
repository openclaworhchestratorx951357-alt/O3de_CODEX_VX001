...
| Validation | `validation.report.intake` | dry-run only (server-gated endpoint candidate, default-off, audited review/status semantics, admission-withheld decision) | read-only (exact public-admission contract + evidence-gated update) | Medium | schema + provenance + fail-closed parser + server-owned authorization gate + explicit public-corridor contract | parser unit tests + endpoint refusal tests + dispatch refusal regression tests + gate-state review/status assertions + admission-contract checks | Validation intake endpoint-candidate exact public-admission contract |
