...
| Validation | `validation.report.intake` | plan-only (contract-designed) | dry-run only | Medium | schema + provenance + server-owned gate + fail-closed parser | contract fixtures + malformed-input fail-closed tests | Validation intake dry-run parser scaffold + fail-closed test matrix |
