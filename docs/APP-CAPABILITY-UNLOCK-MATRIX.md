...
| Validation | `validation.report.intake` | reviewable (default-off server-gated endpoint candidate with admission decision, operator examples, and review checkpoint guidance) | release-readiness decision checkpoint | Medium | schema + provenance + fail-closed parser + server-owned authorization gate + endpoint review/status audit + explicit admission decision + operator-checkpoint guidance | parser unit tests + endpoint gate-state tests + endpoint refusal tests + dispatch refusal regression tests + matrix alignment and operator-guidance evidence | Validation intake endpoint-candidate release-readiness decision/checkpoint packet |
