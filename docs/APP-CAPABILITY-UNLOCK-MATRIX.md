...
| Validation | `validation.report.intake` | dry-run only (server-gated endpoint candidate, default-off) | read-only (audited endpoint candidate) | Medium | schema + provenance + fail-closed parser + server-owned authorization gate | parser unit tests + endpoint refusal tests + dispatch refusal regression tests | Validation intake endpoint-candidate admission audit/review packet |
...
| Validation | `TIAF/preflight` | preflight-only baseline established (plan-only, non-executing) | reviewable preflight contract with execution-design gate references | Medium | no-runtime-mutation gate + explicit execution-admission decision gate | preflight contract tests + artifact checks + admission-design evidence | CI admission design packet |
