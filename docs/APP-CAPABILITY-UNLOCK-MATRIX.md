...
| Validation | `validation.report.intake` | read-only (audited endpoint candidate; server-gated default-off dry-run corridor) | hold boundary; revisit only with explicit approval and new evidence | Medium | schema + provenance + fail-closed parser + server-owned authorization gate | parser unit tests + endpoint refusal tests + dispatch refusal regression tests + audit/review evidence | CI admission design packet |
