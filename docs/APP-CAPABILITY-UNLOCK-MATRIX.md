...
| Validation | `validation.report.intake` | reviewable (default-off server-gated endpoint candidate with admission, operator guidance, and release-readiness long-hold decision) | hold boundary; revisit only with explicit approval and new evidence | Medium | schema + provenance + fail-closed parser + server-owned authorization gate + endpoint review/status audit + explicit admission/release decisions | parser unit tests + endpoint gate-state tests + endpoint refusal tests + dispatch refusal regression tests + matrix/operator/checkpoint evidence | TIAF preflight baseline audit packet |
...
| Validation | `TIAF/preflight` | preflight-only baseline established (plan-only, non-executing) | reviewable preflight contract | Medium | no-runtime-mutation gate + explicit execution-admission decision gate | preflight contract tests + artifact checks + admission-design evidence | CI/test execution admission design packet |
