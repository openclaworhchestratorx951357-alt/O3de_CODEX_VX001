...
| Validation | `validation.report.intake` | dispatch-admission implementation-gated (default-off scaffolding; endpoint-candidate read-only remains admitted) | dispatch-admission decision-checkpoint | Medium | schema + provenance + fail-closed parser + server-owned authorization gate + dispatch boundary design + readiness audit closure + implementation touchpoints | parser unit tests + endpoint gate-state review tests + dispatch refusal regression tests + dispatch gate-state transition tests | Validation intake dispatch-admission decision checkpoint packet |
