...
| Validation | `validation.report.intake` | reviewable (admission-decision confirmed default-off server-gated endpoint candidate; explicit-on dry-run-only) | reviewable plus operator-checkpoint packaging | Medium | schema + provenance + fail-closed parser + server-owned authorization gate + endpoint review/status audit + explicit admission decision | parser unit tests + endpoint gate-state tests + endpoint refusal tests + dispatch refusal regression tests + matrix alignment evidence | Validation intake endpoint-candidate operator examples + review checkpoint packet |
