...
| Validation | `validation.report.intake` | read-only (audited endpoint candidate, server-gated default-off, dry-run-only) | dispatch-admission design-only | Medium | schema + provenance + fail-closed parser + server-owned authorization gate + dispatch boundary design | parser unit tests + endpoint gate-state review tests + dispatch refusal regression tests | Validation intake dispatch-admission design packet |
