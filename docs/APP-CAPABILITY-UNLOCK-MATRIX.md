...
| Validation | `validation.report.intake` | dry-run only (internal parser scaffold; endpoint unadmitted) | read-only (server-gated endpoint candidate) | Medium | schema + provenance + fail-closed parser + server-owned authorization gate | parser unit tests + dispatch refusal regression tests | Validation intake endpoint-candidate dry-run implementation (default-off) |
