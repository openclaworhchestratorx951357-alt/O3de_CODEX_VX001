...
| Validation | `validation.report.intake` | dispatch-registration design-locked (default-off scaffolding; endpoint-candidate read-only remains admitted) | dispatch-registration readiness-audited | Medium | schema + provenance + fail-closed parser + server-owned authorization gate + dispatch design + readiness + implementation + decision checkpoint + catalog registration design | parser unit tests + endpoint gate-state review tests + dispatch refusal regression tests + dispatch gate-state transition tests + catalog registration schema checklist + dispatcher boundary audit | Validation intake dispatch-admission catalog registration readiness audit packet |
