...
| Validation | `validation.report.intake` | dispatch-registration implementation-complete, unadmitted (default-off fail-closed; endpoint-candidate read-only remains admitted) | post-registration admission-contract design | Medium | schema + provenance + fail-closed parser + server-owned authorization gate + dispatch design + readiness + implementation + catalog registration decision checkpoint | parser unit tests + endpoint gate-state review tests + dispatch refusal regression tests + dispatch gate-state transition tests + intake schema set + dispatcher gate-state tests | Validation intake dispatch-admission post-registration admission contract design packet |
