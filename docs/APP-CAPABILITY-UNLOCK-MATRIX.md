...
| Validation | `validation.report.intake` | dry-run only (server-gated endpoint candidate, default-off, audited review/status semantics) | read-only (admission-decision reviewed endpoint candidate) | Medium | schema + provenance + fail-closed parser + server-owned authorization gate | parser unit tests + endpoint refusal tests + dispatch refusal regression tests + gate-state review/status assertions | Validation intake endpoint-candidate admission decision + surface-matrix update |
