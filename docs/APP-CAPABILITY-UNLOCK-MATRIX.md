...
| Validation | `validation.report.intake` | dry-run only (server-gated endpoint candidate, default-off, audited review/status semantics, admission-withheld decision, exact contract defined) | read-only (admission-readiness checklist + evidence-gated update) | Medium | schema + provenance + fail-closed parser + server-owned authorization gate + explicit public-corridor contract + readiness gate mapping | parser unit tests + endpoint refusal tests + dispatch refusal regression tests + gate-state review/status assertions + admission-contract checks + readiness checklist evidence map | Validation intake endpoint-candidate read-only admission readiness checklist |
