...
| Validation | `validation.report.intake` | dispatch-admission design-only (endpoint-candidate read-only remains admitted) | dispatch-admission readiness-audited | Medium | schema + provenance + fail-closed parser + server-owned authorization gate + dispatch boundary design + readiness audit gates | parser unit tests + endpoint gate-state review tests + dispatch refusal regression tests + readiness audit checklist | Validation intake dispatch-admission readiness audit packet |
