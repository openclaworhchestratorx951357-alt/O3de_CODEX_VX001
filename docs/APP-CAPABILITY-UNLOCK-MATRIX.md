...
| Validation | `validation.report.intake` | read-only (audited endpoint candidate, server-gated default-off, dry-run-only) | admission-decision checkpoint | Medium | schema + provenance + fail-closed parser + server-owned authorization gate | parser unit tests + endpoint gate-state review tests + dispatch refusal regression tests | Validation intake endpoint-candidate admission decision surface matrix packet |
