...
| Validation | `validation.report.intake` | dispatch-admission decision-checkpointed (default-off scaffolding; endpoint-candidate read-only remains admitted) | dispatch-registration design-locked | Medium | schema + provenance + fail-closed parser + server-owned authorization gate + dispatch design + readiness + implementation + decision checkpoint closure | parser unit tests + endpoint gate-state review tests + dispatch refusal regression tests + dispatch gate-state transition tests + dispatch registration design checklist | Validation intake dispatch-admission catalog registration design packet |
