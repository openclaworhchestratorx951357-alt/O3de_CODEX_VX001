...
| Validation | `real CI/test execution` | design-gated baseline established (admission not granted) | proof-only admission candidate | High | explicit CI admission decision + allowlist + timeout + provenance + refusal gates | CI surface inventory + gate design + targeted refusal/acceptance tests plan | TIAF preflight baseline audit |
