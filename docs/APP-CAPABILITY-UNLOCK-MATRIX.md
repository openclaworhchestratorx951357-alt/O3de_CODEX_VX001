...
| Validation | `validation.report.intake` | read-only (audited endpoint candidate, server-gated default-off) | reviewable | Medium | schema + provenance + fail-closed parser + server-owned authorization gate + endpoint review/status audit | parser unit tests + endpoint gate-state tests + endpoint refusal tests + dispatch refusal regression tests | Validation intake endpoint-candidate admission decision + surface-matrix update packet |
