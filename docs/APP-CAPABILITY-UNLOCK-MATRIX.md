...
| Validation | `validation.report.intake` | dry-run only (internal parser scaffold; endpoint unadmitted) | read-only (gated endpoint candidate) | Medium | schema + provenance + fail-closed parser + server-owned authorization gate | parser unit tests + dispatch refusal regression tests | Approval/session dashboard shell with intent-only authorization labels |
...
| GUI | `evidence.timeline` | GUI/demo only (static timeline shell) | reviewable | Medium | truth-label contract + audit review | fixture validation + UI rendering tests | Approval/session dashboard shell |
