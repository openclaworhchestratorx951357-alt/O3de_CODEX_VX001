...
| `asset.source.inspect` | M5 real but narrow; project-general discovery helper | surface matrix, Phase 9 proof docs, `docs/PHASE-9-PRODUCTION-GENERALIZATION-PLAN.md`, `docs/PHASE-9-PROJECT-ASSET-READBACK-DISCOVERY.md` | Narrow project-local source metadata plus bounded read-only `Cache/assetdb.sqlite` product/dependency evidence and Asset Catalog product-path presence for one explicit source asset. A read-only discovery helper now models project/source/cache/catalog readiness, but public adapter integration and production-general proof are still pending. |
...
| `ai.asset.o3de.readback.verify` | M1 depends on Phase 9 proof | `docs/PHASE-9-AI-ASSET-FORGE-INTEGRATION.md`, `docs/PHASE-9-PROJECT-ASSET-READBACK-DISCOVERY.md` | Future generated assets must use Phase 9 source/product/dependency/catalog readback. Depends on project-general `asset.source.inspect` proof and schema hardening. |
