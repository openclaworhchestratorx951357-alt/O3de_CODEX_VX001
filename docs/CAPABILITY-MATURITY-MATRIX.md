...
| `validation.report.intake` | M6 reviewable real, default-off server-gated endpoint candidate | `docs/VALIDATION-REPORT-INTAKE-CONTRACT-DESIGN.md`, `docs/VALIDATION-REPORT-INTAKE-DRY-RUN-PARSER-MATRIX.md`, `docs/VALIDATION-REPORT-INTAKE-ENDPOINT-CANDIDATE-DRY-RUN-IMPLEMENTATION.md`, `docs/VALIDATION-REPORT-INTAKE-ENDPOINT-CANDIDATE-ADMISSION-AUDIT-REVIEW.md`, `docs/VALIDATION-REPORT-INTAKE-ENDPOINT-CANDIDATE-ADMISSION-DECISION.md`, surface matrix, validation-report-intake route/parser tests | `POST /validation/report/intake` is reviewable and server-gated default-off. Gate states fail closed when missing/off/invalid; explicit-on remains dry-run-only with no execution/mutation admission and with explicit refusal reasons. `/tools/dispatch` remains unadmitted for this capability. |
