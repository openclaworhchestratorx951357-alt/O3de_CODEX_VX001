...
| `asset.source.inspect` | M5 real but narrow; project-general candidate | surface matrix, Phase 9 proof docs, `docs/PHASE-9-PRODUCTION-GENERALIZATION-PLAN.md` | Narrow project-local source metadata plus bounded read-only `Cache/assetdb.sqlite` product/dependency evidence and Asset Catalog product-path presence for one explicit source asset. Live proof is on `McpSandbox`/`BridgeLevel01`; production-general project discovery and readiness states are still pending. |
