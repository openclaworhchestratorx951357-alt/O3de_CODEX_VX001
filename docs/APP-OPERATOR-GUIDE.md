...
- The lifecycle command restarts or reuses the canonical backend and can launch the canonical McpSandbox Editor bridge when no canonical Editor is already running.
...
- Latest bundle pattern is backend\runtime\live_editor_entity_exists_proof_<timestamp>.json; latest observed bundle was backend\runtime\live_editor_entity_exists_proof_20260425-094047.json.
- The proof helper creates a Prompt Studio session and requires the exact editor.session.open -> editor.level.open -> editor.entity.exists direct read-only plan before execution.
...
- editor.entity.exists returned exists = true, lookup_mode = entity_name, matched_count = 1, and entity_id = [6840328185757485916].
- Prompt proof session was editor-entity-exists-proof-20260425-094047 with 3 execute attempts and 2 approvals.
- Recorded lineage: session run_id = run-e7eab183ed5c, level run_id = run-bec4acd1da55, exists run_id = run-a3d5b97ed9cf.
- Recorded exists evidence: execution_id = exe-0c54b90c1189, artifact_id = art-601e49da7e6f, bridge_command_id = 93a0d51206d1464ea3c1846b157387a4.
...
- Prompt Studio direct editor.entity.exists prompts now plan editor.level.open with make_writable=false and focus_viewport=false, then execute only the admitted exact lookup.
...
- Operator-facing editor-chain and direct entity-exists review summaries now distinguish requested action, executed action, verified facts, assumptions, missing proof, and safest next step.
...
- Bounded direct entity-exists review labels now include succeeded_readback_verified, succeeded_absence_verified, blocked_missing_editor_target, blocked_missing_level, blocked_ambiguous_entity_lookup, failed_runtime_error, and incomplete_readback_unavailable.
...
- Prompt Studio can also plan direct read-only editor.entity.exists reviews through editor.session.open, read-only editor.level.open, and exact entity-id or exact entity-name lookup without adding editor mutation.
- The repo-owned standalone editor.entity.exists proof helper now uses Prompt Studio so the proof verifies the exact direct plan, approval walk, child lineage, exact-name readback, and bounded final review summary.
- The latest direct editor.entity.exists proof verifies exact-name readback for Ground on Levels/TestLoevel01 through Prompt Studio and does not prove live Editor undo, viewport reload, cleanup, broad entity discovery, or entity absence after file-backed restore.
...
- Treat direct entity-exists review summaries as exact read-only readback evidence only; they do not claim cleanup, restore, mutation, or reversibility.
