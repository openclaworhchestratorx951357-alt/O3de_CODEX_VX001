...
This guide gives operators safe prompt wording for inspecting or changing the
one admitted Camera bool target without implying broad component-property
editing support.
...
This is not generalized undo. A write review may tell the operator how to rerun
the same exact corridor with the recorded previous value, or how to use a
recorded loaded-level restore boundary outside the public corridor when one is
available. Do not treat this as live Editor undo, viewport reload, or broad
cleanup.
