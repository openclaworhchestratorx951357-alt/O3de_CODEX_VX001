...
def test_build_compile_is_cataloged_as_execution_gated() -> None:
    tool = catalog_service.get_tool_definition("project-build", "build.compile")
    assert tool is not None
    assert tool.approval_class == "build_execute"
    assert tool.adapter_family == "project-build"
    assert tool.capability_status == "execution-gated"
...
def test_exact_camera_bool_property_write_is_cataloged_as_mutation_gated() -> None:
    tool = catalog_service.get_tool_definition(
        "editor-control",
    ...
    assert tool.args_schema.endswith(
        "editor.component.property.write.camera_bool_make_active_on_activation.args.schema.json"
    )
...
def test_editor_entity_exists_is_cataloged_as_hybrid_read_only() -> None:
    ...
    assert tool.adapter_family == "editor-control"
    assert tool.capability_status == "hybrid-read-only"
    assert tool.args_schema.endswith("editor.entity.exists.args.schema.json")
    assert tool.result_schema.endswith("editor.entity.exists.result.schema.json")
    assert "editor_session" in tool.default_locks
