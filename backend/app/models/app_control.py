...
AppControlReportMode = Literal["applied", "reverted"]
AppControlVerification = Literal["verified", "assumed"]
...
class AppControlExecutionReportRequest(BaseModel):
    script_id: str = Field(..., min_length=1)
...
class AppControlExecutionReportItem(BaseModel):
...
class AppControlExecutionReport(BaseModel):
    script_id: str = Field(..., min_length=1)
    mode: AppControlReportMode
    summary: str = Field(..., min_length=1)
    items: list[AppControlExecutionReportItem] = Field(default_factory=list)
    event_id: str | None = None
    generated_by: str = "deterministic-app-control-report-v1"
