    ...
    tools: [
      {
        ...
        enabled: true,
        selected: true,
        description: "UI-only tool selection",
        blocked_reason: null,
        next_unlock: null,
...
describe("AIAssetForgePanel", () => {
  it("fetches the backend editor model and renders the cockpit shell", async () => {
    apiMocks.fetchAssetForgeEditorModel.mockResolvedValueOnce(buildEditorModelFixture());
  ...
  it("forwards prompt-template prefill callback from the cockpit", async () => {
    apiMocks.fetchAssetForgeEditorModel.mockResolvedValueOnce(buildEditorModelFixture());
    const onPrefillPromptTemplate = vi.fn();

    render(<AIAssetForgePanel onPrefillPromptTemplate={onPrefillPromptTemplate} />);

    const prefillButton = await screen.findByRole("button", {
      name: "Prefill linked template in Prompt Studio (no auto-execute)",
    });
