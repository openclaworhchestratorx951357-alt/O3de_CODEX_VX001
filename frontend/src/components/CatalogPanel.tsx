...
export default function CatalogPanel({ agents }: CatalogPanelProps) {
  function describeCapability(toolName: string, capabilityStatus?: string): string {
    if (capabilityStatus === "hybrid-read-only" && toolName === "project.inspect") {
      return "Real read-only in hybrid mode when manifest preconditions are satisfied, with manifest-backed project-config, requested-vs-discovered Gem evidence, requested Gem subset matching, requested settings subset matching, and top-level settings evidence.";
    }
