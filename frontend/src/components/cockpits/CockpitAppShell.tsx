...
import { getCockpitDefinition } from "./registry/cockpitRegistry";
...
export default function CockpitAppShell({
  cockpitId,
  title,
  subtitle,
  truthLabel,
  ...
  const panelDefinitions: CockpitPanelDefinition[] = [
    {
      ...
      render: () => (
        <section style={identityCardStyle}>
          ...
          {definition ? (
            <p style={detailStyle}><strong>Registry route:</strong> {definition.routeKey}</p>
          ) : null}
        </section>
      ),
    },
  ...
  if (definition?.panels?.length) {
    panelDefinitions.push({
      id: "registry-layout-blueprint",
      title: "Cockpit layout blueprint",
      subtitle: "Registry-driven zone map for this cockpit",
      truthState: "status",
      defaultZone: "bottom",
      collapsible: true,
      scrollMode: "content",
      ...
      render: () => (
        <ul style={listStyle}>
          {definition.panels.map((panel) => (
            <li key={panel.id}>
              <strong>{panel.title}</strong>: {panel.zone} zone ({panel.contentType})
            </li>
          ))}
        </ul>
      ),
  ...
  return (
    <DesktopWindow
      ...
      subtitle={definition?.subtitle || subtitle}
    ...
    >
      <DockableCockpitLayout cockpitId={cockpitId} panels={panelDefinitions} />
    </DesktopWindow>
  );
}
