...
describe("cockpitLayoutStore", () => {
  ...
  it("uses the provided default preset when no saved layout exists", () => {
    ...
    expect(loaded.sizes.leftPrimaryRatio).toBeCloseTo(0.25);
    expect(loaded.sizes.centerPrimaryRatio).toBeCloseTo(0.81);
    expect(loaded.sizes.topPrimaryRatio).toBeCloseTo(0.85);
  });
