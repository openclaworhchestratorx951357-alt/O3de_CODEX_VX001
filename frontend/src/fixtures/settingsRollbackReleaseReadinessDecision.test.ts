...
describe("build execution long-hold fixture guardrails", () => {
  it("keeps build execution capability boundaries explicit and non-broadening", () => {
    ...
    expect(buildExecutionCapability?.recommendedNextPacket).toBe("Editor placement proof-only implementation");
    ...
    expect(buildExecutionAudit?.nextGate).toBe("Editor placement proof-only implementation");
    ...
    expect(buildExecutionStatus?.nextGate).toBe("Editor placement proof-only implementation");
  });
