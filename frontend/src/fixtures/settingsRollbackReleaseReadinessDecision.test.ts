...
describe("build execution long-hold fixture guardrails", () => {
  it("keeps build execution capability boundaries explicit and non-broadening", () => {
    ...
    expect(buildExecutionCapability?.recommendedNextPacket).toBe("Asset Forge placement proof-only admission-flag design");
    ...
    expect(buildExecutionAudit?.nextGate).toBe("Asset Forge placement proof-only admission-flag design");
    ...
    expect(buildExecutionStatus?.nextGate).toBe("Asset Forge placement proof-only admission-flag design");
  });
