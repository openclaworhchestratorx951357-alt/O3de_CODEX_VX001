...
describe("operatorGuide catalog", () => {
  it("keeps the guided surface inventory in sync with the current desktop and panel coverage", () => {
    ...
    expect(operatorGuideCatalog.proofChecklist[5]?.evidence[3]).toMatch(/position remain rejected/i);
    expect(operatorGuideCatalog.proofChecklist[5]?.evidence[7]).toMatch(/editor\.component\.add is admitted real-authoring/i);
    expect(operatorGuideCatalog.proofChecklist[5]?.evidence[9]).toMatch(/editor\.component\.property\.get is admitted hybrid read-only/i);
