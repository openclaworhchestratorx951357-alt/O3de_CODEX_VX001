...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-operator-activation-receipt-lane-continuity-warrant-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Activation-Receipt-Lane-Continuity-Warrant.ps1'
  $reportPath = Join-Path $testQueue 'operator-activation-receipt-lane-continuity-warrant.json'
  ...
  Assert-True ($readyReport.continuity_warrant_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.continuity_warrant_complete) 'acknowledged scenario continuity_warrant_complete should be true'
  ...
  Assert-True ($attentionReport.continuity_warrant_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.continuity_warrant_complete) 'non-acknowledged scenario continuity_warrant_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence operator activation receipt lane continuity warrant checks.' -ForegroundColor Green
}
finally {
