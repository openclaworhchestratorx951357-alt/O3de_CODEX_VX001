param(
  ...
  [string]$LedgerName = 'stewardship_bulletin_custody_gazette',
...
if (-not $SealScriptPath) {
  $SealScriptPath = Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Operator-Continuity-Docket-Stewardship-Bulletin.ps1'
}
if (-not $SealReportPath) {
  $SealReportPath = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-operator-continuity-docket-stewardship-bulletin.json'
}
if (-not $OutputFile) {
  $OutputFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-stewardship-bulletin-custody-gazette.json'
}
if (-not $OutputTextFile) {
  $OutputTextFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-stewardship-bulletin-custody-gazette.txt'
}
...
$recommendedAction = 'publish_stewardship_bulletin_custody_gazette'
$sealRan = $false

if (-not (Test-Path $SealScriptPath)) {
  Add-Finding -Severity 'warning' -Code 'lane_custody_ledger_script_missing' -Message 'Operator continuity docket stewardship bulletin script missing.' -Path $SealScriptPath
}
else {
  $cmd = @(
    '-NoLogo',
  ...
  if (-not $sealRan) {
    Add-Finding -Severity 'warning' -Code 'lane_custody_ledger_execution_failed' -Message 'Stewardship bulletin execution failed.' -Path $SealScriptPath
  }
}
...
if (Test-Path $SealReportPath) {
  try {
    ...
    if ($sealData.stewardship_bulletin_state) {
      $operatorHandoffSealState = [string]$sealData.stewardship_bulletin_state
    }
    if ($null -ne $sealData.stewardship_bulletin_complete) {
      $operatorHandoffSealComplete = [bool]$sealData.stewardship_bulletin_complete
    }
    ...
    if ($sealData.stewardship_bulletin) {
      $operatorHandoffSeal = @($sealData.stewardship_bulletin)
    }
  }
  catch {
  Add-Finding -Severity 'warning' -Code 'stewardship_bulletin_report_parse_failed' -Message "Unable to parse stewardship bulletin report JSON: $($_.Exception.Message)" -Path $SealReportPath
  }
}
else {
  Add-Finding -Severity 'warning' -Code 'stewardship_bulletin_report_missing' -Message 'Stewardship bulletin report missing after custody gazette execution.' -Path $SealReportPath
}

$laneCustodyLedger = @(
  'custody_gazette_basis=stewardship_bulletin',
  ...
  "stewardship_bulletin_count=$(@($operatorHandoffSeal).Count)",
  "operator_acknowledged=$([bool]$OperatorAcknowledged)",
  'custody_gazette_note=Custody gazette is recorded as local governance evidence only.'
)
...
if (-not $laneCustodyLedgerComplete) {
  ...
  $recommendedAction = 'repair_stewardship_bulletin_and_rebuild_custody_gazette'
  Add-Finding -Severity 'warning' -Code 'custody_gazette_not_ready' -Message 'Stewardship bulletin custody gazette checks failed.' -Path $SealReportPath
}
...
$report = [ordered]@{
  ...
  operator_acknowledged = [bool]$OperatorAcknowledged
  custody_gazette_state = $laneCustodyLedgerState
  custody_gazette_complete = $laneCustodyLedgerComplete
  recommended_action = $recommendedAction
  seal_script_path = $SealScriptPath
  seal_report_path = $SealReportPath
  seal_ran = $sealRan
  stewardship_bulletin_state = $operatorHandoffSealState
  stewardship_bulletin_complete = $operatorHandoffSealComplete
  lane_profile_name = $laneProfileName
  profile_state = $profileState
  launch_command = $launchCommand
  ...
  stewardship_bulletin = $operatorHandoffSeal
  custody_gazette = $laneCustodyLedger
  local_only_note = 'Stewardship bulletin custody gazette is governance-only and does not authorize runtime execution, provider expansion, placement, or project mutation.'
  findings = $findings
}
