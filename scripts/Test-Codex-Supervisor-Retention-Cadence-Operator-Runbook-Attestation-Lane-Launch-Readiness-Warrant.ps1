...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-operator-runbook-attestation-lane-launch-readiness-warrant-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Runbook-Attestation-Lane-Launch-Readiness-Warrant.ps1'
  $reportPath = Join-Path $testQueue 'operator-runbook-attestation-lane-launch-readiness-warrant.json'
  ...
  Assert-True ($readyReport.warrant_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.warrant_complete) 'acknowledged scenario warrant_complete should be true'
  ...
  Assert-True ($attentionReport.warrant_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.warrant_complete) 'non-acknowledged scenario warrant_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence operator runbook attestation lane launch readiness warrant checks.' -ForegroundColor Green
}
finally {
