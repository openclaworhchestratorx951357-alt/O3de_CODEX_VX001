...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-handoff-index-launch-playbook-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Handoff-Index-Launch-Playbook.ps1'
  $reportPath = Join-Path $testQueue 'handoff-index-launch-playbook.json'
  ...
  Assert-True ($readyReport.playbook_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.playbook_complete) 'acknowledged scenario playbook_complete should be true'
  ...
  Assert-True ($attentionReport.playbook_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.playbook_complete) 'non-acknowledged scenario playbook_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence handoff index launch playbook checks.' -ForegroundColor Green
}
finally {
