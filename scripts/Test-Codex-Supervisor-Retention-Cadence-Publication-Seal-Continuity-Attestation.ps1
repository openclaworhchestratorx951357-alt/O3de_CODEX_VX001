...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-publication-seal-continuity-attestation-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Publication-Seal-Continuity-Attestation.ps1'
  $reportPath = Join-Path $testQueue 'publication-seal-continuity-attestation.json'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence publication seal continuity attestation checks.' -ForegroundColor Green
}
finally {
