...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-continuity-checkpoint-handoff-digest-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Continuity-Checkpoint-Handoff-Digest.ps1'
  $reportPath = Join-Path $testQueue 'continuity-checkpoint-handoff-digest.json'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence continuity checkpoint handoff digest checks.' -ForegroundColor Green
}
finally {
