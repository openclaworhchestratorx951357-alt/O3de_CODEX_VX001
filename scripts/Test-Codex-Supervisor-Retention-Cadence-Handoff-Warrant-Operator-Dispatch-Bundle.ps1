...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-handoff-warrant-operator-dispatch-bundle-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Handoff-Warrant-Operator-Dispatch-Bundle.ps1'
  $reportPath = Join-Path $testQueue 'handoff-warrant-operator-dispatch-bundle.json'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence handoff warrant operator dispatch bundle checks.' -ForegroundColor Green
}
finally {
