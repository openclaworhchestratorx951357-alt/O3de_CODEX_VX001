...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-startup-lane-profile-adoption-handoff-bundle-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Startup-Lane-Profile-Adoption-Handoff-Bundle.ps1'
  $reportPath = Join-Path $testQueue 'startup-lane-profile-adoption-handoff-bundle.json'
  ...
  Assert-True ($readyReport.bundle_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.bundle_complete) 'acknowledged scenario bundle_complete should be true'
  ...
  Assert-True ($attentionReport.bundle_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.bundle_complete) 'non-acknowledged scenario bundle_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence startup lane profile adoption handoff bundle checks.' -ForegroundColor Green
}
finally {
