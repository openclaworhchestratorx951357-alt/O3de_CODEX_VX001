param(
  ...
  [string]$LedgerName = 'operator_continuity_docket_stewardship_bulletin',
...
if (-not $SealScriptPath) {
  $SealScriptPath = Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Lane-Custody-Ledger-Operator-Continuity-Docket.ps1'
}
if (-not $SealReportPath) {
  $SealReportPath = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-lane-custody-ledger-operator-continuity-docket.json'
}
if (-not $OutputFile) {
  $OutputFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-operator-continuity-docket-stewardship-bulletin.json'
}
if (-not $OutputTextFile) {
  $OutputTextFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-operator-continuity-docket-stewardship-bulletin.txt'
}
...
$recommendedAction = 'publish_operator_continuity_docket_stewardship_bulletin'
$sealRan = $false

if (-not (Test-Path $SealScriptPath)) {
  Add-Finding -Severity 'warning' -Code 'lane_custody_ledger_script_missing' -Message 'Lane custody ledger operator continuity docket script missing.' -Path $SealScriptPath
}
else {
  $cmd = @(
    '-NoLogo',
  ...
  if (-not $sealRan) {
    Add-Finding -Severity 'warning' -Code 'lane_custody_ledger_execution_failed' -Message 'Operator continuity docket execution failed.' -Path $SealScriptPath
  }
}
...
if (Test-Path $SealReportPath) {
  try {
    ...
    if ($sealData.operator_continuity_docket_state) {
      $operatorHandoffSealState = [string]$sealData.operator_continuity_docket_state
    }
    if ($null -ne $sealData.operator_continuity_docket_complete) {
      $operatorHandoffSealComplete = [bool]$sealData.operator_continuity_docket_complete
    }
    ...
    if ($sealData.operator_continuity_docket) {
      $operatorHandoffSeal = @($sealData.operator_continuity_docket)
    }
  }
  catch {
  Add-Finding -Severity 'warning' -Code 'operator_continuity_docket_report_parse_failed' -Message "Unable to parse operator continuity docket report JSON: $($_.Exception.Message)" -Path $SealReportPath
  }
}
else {
  Add-Finding -Severity 'warning' -Code 'operator_continuity_docket_report_missing' -Message 'Operator continuity docket report missing after stewardship bulletin execution.' -Path $SealReportPath
}

$laneCustodyLedger = @(
  'stewardship_bulletin_basis=operator_continuity_docket',
  ...
  "operator_continuity_docket_count=$(@($operatorHandoffSeal).Count)",
  "operator_acknowledged=$([bool]$OperatorAcknowledged)",
  'stewardship_bulletin_note=Stewardship bulletin is recorded as local governance evidence only.'
)
...
if (-not $laneCustodyLedgerComplete) {
  ...
  $recommendedAction = 'repair_operator_continuity_docket_and_rebuild_stewardship_bulletin'
  Add-Finding -Severity 'warning' -Code 'stewardship_bulletin_not_ready' -Message 'Operator continuity docket stewardship bulletin checks failed.' -Path $SealReportPath
}
...
$report = [ordered]@{
  ...
  operator_acknowledged = [bool]$OperatorAcknowledged
  stewardship_bulletin_state = $laneCustodyLedgerState
  stewardship_bulletin_complete = $laneCustodyLedgerComplete
  recommended_action = $recommendedAction
  seal_script_path = $SealScriptPath
  seal_report_path = $SealReportPath
  seal_ran = $sealRan
  operator_continuity_docket_state = $operatorHandoffSealState
  operator_continuity_docket_complete = $operatorHandoffSealComplete
  lane_profile_name = $laneProfileName
  profile_state = $profileState
  launch_command = $launchCommand
  ...
  operator_continuity_docket = $operatorHandoffSeal
  stewardship_bulletin = $laneCustodyLedger
  local_only_note = 'Operator continuity docket stewardship bulletin is governance-only and does not authorize runtime execution, provider expansion, placement, or project mutation.'
  findings = $findings
}
