...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-handoff-digest-readiness-ledger-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Handoff-Digest-Readiness-Ledger.ps1'
  $reportPath = Join-Path $testQueue 'handoff-digest-readiness-ledger.json'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence handoff digest readiness ledger checks.' -ForegroundColor Green
}
finally {
