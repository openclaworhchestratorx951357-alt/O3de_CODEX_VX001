param(
  ...
  [string]$LedgerName = 'continuity_addendum_publication_docket',
...
if (-not $SealScriptPath) {
  $SealScriptPath = Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Operator-Governance-Digest-Continuity-Addendum.ps1'
}
if (-not $SealReportPath) {
  $SealReportPath = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-operator-governance-digest-continuity-addendum.json'
}
if (-not $OutputFile) {
  $OutputFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-continuity-addendum-publication-docket.json'
}
if (-not $OutputTextFile) {
  $OutputTextFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-continuity-addendum-publication-docket.txt'
}
...
$recommendedAction = 'publish_continuity_addendum_publication_docket'
$sealRan = $false

if (-not (Test-Path $SealScriptPath)) {
  Add-Finding -Severity 'warning' -Code 'continuity_addendum_script_missing' -Message 'Operator governance digest continuity addendum script missing.' -Path $SealScriptPath
}
else {
  $cmd = @(
    '-NoLogo',
  ...
  if (-not $sealRan) {
    Add-Finding -Severity 'warning' -Code 'continuity_addendum_execution_failed' -Message 'Continuity addendum execution failed.' -Path $SealScriptPath
  }
}
...
if (Test-Path $SealReportPath) {
  try {
    ...
    if ($sealData.operator_governance_digest_continuity_addendum_state) {
      $operatorHandoffSealState = [string]$sealData.operator_governance_digest_continuity_addendum_state
    }
    if ($null -ne $sealData.operator_governance_digest_continuity_addendum_complete) {
      $operatorHandoffSealComplete = [bool]$sealData.operator_governance_digest_continuity_addendum_complete
    }
    ...
    if ($sealData.operator_governance_digest_continuity_addendum) {
      $operatorHandoffSeal = @($sealData.operator_governance_digest_continuity_addendum)
    }
  }
  catch {
  Add-Finding -Severity 'warning' -Code 'continuity_addendum_report_parse_failed' -Message "Unable to parse continuity addendum report JSON: $($_.Exception.Message)" -Path $SealReportPath
  }
}
else {
  Add-Finding -Severity 'warning' -Code 'continuity_addendum_report_missing' -Message 'Continuity addendum report missing after publication docket execution.' -Path $SealReportPath
}

$laneCustodyLedger = @(
  'publication_docket_basis=continuity_addendum',
  ...
  "continuity_addendum_count=$(@($operatorHandoffSeal).Count)",
  "operator_acknowledged=$([bool]$OperatorAcknowledged)",
  'publication_docket_note=Publication docket is recorded as local governance evidence only.'
)
...
if (-not $laneCustodyLedgerComplete) {
  ...
  $recommendedAction = 'repair_continuity_addendum_and_rebuild_publication_docket'
  Add-Finding -Severity 'warning' -Code 'operator_governance_digest_not_ready' -Message 'Continuity addendum publication docket checks failed.' -Path $SealReportPath
}
...
$report = [ordered]@{
  ...
  operator_acknowledged = [bool]$OperatorAcknowledged
  continuity_addendum_publication_docket_state = $laneCustodyLedgerState
  continuity_addendum_publication_docket_complete = $laneCustodyLedgerComplete
  recommended_action = $recommendedAction
  seal_script_path = $SealScriptPath
  seal_report_path = $SealReportPath
  seal_ran = $sealRan
  operator_governance_digest_continuity_addendum_state = $operatorHandoffSealState
  operator_governance_digest_continuity_addendum_complete = $operatorHandoffSealComplete
  lane_profile_name = $laneProfileName
  profile_state = $profileState
  launch_command = $launchCommand
  ...
  operator_governance_digest_continuity_addendum = $operatorHandoffSeal
  continuity_addendum_publication_docket = $laneCustodyLedger
  local_only_note = 'Continuity addendum publication docket is governance-only and does not authorize runtime execution, provider expansion, placement, or project mutation.'
  findings = $findings
}
