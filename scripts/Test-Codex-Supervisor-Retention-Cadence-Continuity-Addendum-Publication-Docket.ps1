...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-continuity-addendum-publication-docket-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  Copy-Item -LiteralPath (Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Operator-Handoff-Seal-Lane-Custody-Ledger.ps1') -Destination (Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Handoff-Seal-Lane-Custody-Ledger.ps1') -Force
  Copy-Item -LiteralPath (Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Lane-Custody-Ledger-Operator-Continuity-Docket.ps1') -Destination (Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Lane-Custody-Ledger-Operator-Continuity-Docket.ps1') -Force
  Copy-Item -LiteralPath (Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Operator-Continuity-Docket-Stewardship-Bulletin.ps1') -Destination (Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Continuity-Docket-Stewardship-Bulletin.ps1') -Force
  Copy-Item -LiteralPath (Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Stewardship-Bulletin-Custody-Gazette.ps1') -Destination (Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Stewardship-Bulletin-Custody-Gazette.ps1') -Force
  Copy-Item -LiteralPath (Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Custody-Gazette-Operator-Ledger-Bulletin.ps1') -Destination (Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Custody-Gazette-Operator-Ledger-Bulletin.ps1') -Force
  Copy-Item -LiteralPath (Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Operator-Ledger-Bulletin-Custody-Register.ps1') -Destination (Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Ledger-Bulletin-Custody-Register.ps1') -Force
  Copy-Item -LiteralPath (Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Custody-Register-Operator-Governance-Digest.ps1') -Destination (Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Custody-Register-Operator-Governance-Digest.ps1') -Force
  Copy-Item -LiteralPath (Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Operator-Governance-Digest-Continuity-Addendum.ps1') -Destination (Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Governance-Digest-Continuity-Addendum.ps1') -Force
  Copy-Item -LiteralPath (Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Continuity-Addendum-Publication-Docket.ps1') -Destination (Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Continuity-Addendum-Publication-Docket.ps1') -Force

  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Continuity-Addendum-Publication-Docket.ps1'
  $reportPath = Join-Path $testQueue 'continuity-addendum-publication-docket.json'
  ...
  Assert-True ($readyReport.continuity_addendum_publication_docket_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.continuity_addendum_publication_docket_complete) 'acknowledged scenario stewardship_receipt_complete should be true'
  ...
  Assert-True ($attentionReport.continuity_addendum_publication_docket_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.continuity_addendum_publication_docket_complete) 'non-acknowledged scenario stewardship_receipt_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence continuity addendum publication docket checks.' -ForegroundColor Green
}
finally {
