...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-operator-stewardship-receipt-lane-custody-attestation-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Stewardship-Receipt-Lane-Custody-Attestation.ps1'
  $reportPath = Join-Path $testQueue 'operator-stewardship-receipt-lane-custody-attestation.json'
  ...
  Assert-True ($readyReport.lane_custody_attestation_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.lane_custody_attestation_complete) 'acknowledged scenario stewardship_receipt_complete should be true'
  ...
  Assert-True ($attentionReport.lane_custody_attestation_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.lane_custody_attestation_complete) 'non-acknowledged scenario stewardship_receipt_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence operator stewardship receipt lane custody attestation checks.' -ForegroundColor Green
}
finally {
