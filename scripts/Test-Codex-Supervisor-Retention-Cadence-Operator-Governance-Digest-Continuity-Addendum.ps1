...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-operator-governance-digest-continuity-addendum-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Governance-Digest-Continuity-Addendum.ps1'
  $reportPath = Join-Path $testQueue 'operator-governance-digest-continuity-addendum.json'
  ...
  Assert-True ($readyReport.operator_governance_digest_continuity_addendum_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.operator_governance_digest_continuity_addendum_complete) 'acknowledged scenario stewardship_receipt_complete should be true'
  ...
  Assert-True ($attentionReport.operator_governance_digest_continuity_addendum_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.operator_governance_digest_continuity_addendum_complete) 'non-acknowledged scenario stewardship_receipt_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence operator governance digest continuity addendum checks.' -ForegroundColor Green
}
finally {
