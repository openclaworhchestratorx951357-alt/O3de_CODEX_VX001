param(
  ...
  [string]$LedgerName = 'custody_gazette_operator_ledger_bulletin',
...
if (-not $SealScriptPath) {
  $SealScriptPath = Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Stewardship-Bulletin-Custody-Gazette.ps1'
}
if (-not $SealReportPath) {
  $SealReportPath = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-stewardship-bulletin-custody-gazette.json'
}
if (-not $OutputFile) {
  $OutputFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-custody-gazette-operator-ledger-bulletin.json'
}
if (-not $OutputTextFile) {
  $OutputTextFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-custody-gazette-operator-ledger-bulletin.txt'
}
...
$recommendedAction = 'publish_custody_gazette_operator_ledger_bulletin'
$sealRan = $false

if (-not (Test-Path $SealScriptPath)) {
  Add-Finding -Severity 'warning' -Code 'lane_custody_ledger_script_missing' -Message 'Stewardship bulletin custody gazette script missing.' -Path $SealScriptPath
}
else {
  $cmd = @(
    '-NoLogo',
  ...
  if (-not $sealRan) {
    Add-Finding -Severity 'warning' -Code 'lane_custody_ledger_execution_failed' -Message 'Custody gazette execution failed.' -Path $SealScriptPath
  }
}
...
if (Test-Path $SealReportPath) {
  try {
    ...
    if ($sealData.custody_gazette_state) {
      $operatorHandoffSealState = [string]$sealData.custody_gazette_state
    }
    if ($null -ne $sealData.custody_gazette_complete) {
      $operatorHandoffSealComplete = [bool]$sealData.custody_gazette_complete
    }
    ...
    if ($sealData.custody_gazette) {
      $operatorHandoffSeal = @($sealData.custody_gazette)
    }
  }
  catch {
  Add-Finding -Severity 'warning' -Code 'custody_gazette_report_parse_failed' -Message "Unable to parse custody gazette report JSON: $($_.Exception.Message)" -Path $SealReportPath
  }
}
else {
  Add-Finding -Severity 'warning' -Code 'custody_gazette_report_missing' -Message 'Custody gazette report missing after operator ledger bulletin execution.' -Path $SealReportPath
}

$laneCustodyLedger = @(
  'operator_ledger_bulletin_basis=custody_gazette',
  ...
  "custody_gazette_count=$(@($operatorHandoffSeal).Count)",
  "operator_acknowledged=$([bool]$OperatorAcknowledged)",
  'operator_ledger_bulletin_note=Operator ledger bulletin is recorded as local governance evidence only.'
)
...
if (-not $laneCustodyLedgerComplete) {
  ...
  $recommendedAction = 'repair_custody_gazette_and_rebuild_operator_ledger_bulletin'
  Add-Finding -Severity 'warning' -Code 'operator_ledger_bulletin_not_ready' -Message 'Custody gazette operator ledger bulletin checks failed.' -Path $SealReportPath
}
...
$report = [ordered]@{
  ...
  operator_acknowledged = [bool]$OperatorAcknowledged
  operator_ledger_bulletin_state = $laneCustodyLedgerState
  operator_ledger_bulletin_complete = $laneCustodyLedgerComplete
  recommended_action = $recommendedAction
  seal_script_path = $SealScriptPath
  seal_report_path = $SealReportPath
  seal_ran = $sealRan
  custody_gazette_state = $operatorHandoffSealState
  custody_gazette_complete = $operatorHandoffSealComplete
  lane_profile_name = $laneProfileName
  profile_state = $profileState
  launch_command = $launchCommand
  ...
  custody_gazette = $operatorHandoffSeal
  operator_ledger_bulletin = $laneCustodyLedger
  local_only_note = 'Custody gazette operator ledger bulletin is governance-only and does not authorize runtime execution, provider expansion, placement, or project mutation.'
  findings = $findings
}
