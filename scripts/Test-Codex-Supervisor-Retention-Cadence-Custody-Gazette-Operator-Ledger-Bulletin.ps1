...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-custody-gazette-operator-ledger-bulletin-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Custody-Gazette-Operator-Ledger-Bulletin.ps1'
  $reportPath = Join-Path $testQueue 'custody-gazette-operator-ledger-bulletin.json'
  ...
  Assert-True ($readyReport.operator_ledger_bulletin_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.operator_ledger_bulletin_complete) 'acknowledged scenario stewardship_receipt_complete should be true'
  ...
  Assert-True ($attentionReport.operator_ledger_bulletin_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.operator_ledger_bulletin_complete) 'non-acknowledged scenario stewardship_receipt_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence custody gazette operator ledger bulletin checks.' -ForegroundColor Green
}
finally {
