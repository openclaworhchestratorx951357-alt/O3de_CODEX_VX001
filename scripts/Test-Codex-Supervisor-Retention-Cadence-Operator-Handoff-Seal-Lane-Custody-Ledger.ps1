...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-operator-handoff-seal-lane-custody-ledger-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Handoff-Seal-Lane-Custody-Ledger.ps1'
  $reportPath = Join-Path $testQueue 'operator-handoff-seal-lane-custody-ledger.json'
  ...
  Assert-True ($readyReport.lane_custody_ledger_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.lane_custody_ledger_complete) 'acknowledged scenario stewardship_receipt_complete should be true'
  ...
  Assert-True ($attentionReport.lane_custody_ledger_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.lane_custody_ledger_complete) 'non-acknowledged scenario stewardship_receipt_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence operator handoff seal lane custody ledger checks.' -ForegroundColor Green
}
finally {
