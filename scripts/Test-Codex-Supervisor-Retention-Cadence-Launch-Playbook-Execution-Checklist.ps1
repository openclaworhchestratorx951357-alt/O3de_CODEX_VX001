...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-launch-playbook-execution-checklist-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Launch-Playbook-Execution-Checklist.ps1'
  $reportPath = Join-Path $testQueue 'launch-playbook-execution-checklist.json'
  ...
  Assert-True ($readyReport.checklist_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.checklist_complete) 'acknowledged scenario checklist_complete should be true'
  ...
  Assert-True ($attentionReport.checklist_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.checklist_complete) 'non-acknowledged scenario checklist_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence launch playbook execution checklist checks.' -ForegroundColor Green
}
finally {
