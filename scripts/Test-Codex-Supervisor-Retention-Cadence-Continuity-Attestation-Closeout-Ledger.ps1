...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-continuity-attestation-closeout-ledger-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Continuity-Attestation-Closeout-Ledger.ps1'
  $reportPath = Join-Path $testQueue 'continuity-attestation-closeout-ledger.json'
  ...
  Assert-True ($readyReport.ledger_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.ledger_complete) 'acknowledged scenario ledger_complete should be true'
  ...
  Assert-True ($attentionReport.ledger_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.ledger_complete) 'non-acknowledged scenario ledger_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence continuity attestation closeout ledger checks.' -ForegroundColor Green
}
finally {
