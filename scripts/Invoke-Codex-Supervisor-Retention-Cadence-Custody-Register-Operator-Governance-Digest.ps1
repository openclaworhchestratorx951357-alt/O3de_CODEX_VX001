param(
  ...
  [string]$LedgerName = 'custody_register_operator_governance_digest',
...
if (-not $SealScriptPath) {
  $SealScriptPath = Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Operator-Ledger-Bulletin-Custody-Register.ps1'
}
if (-not $SealReportPath) {
  $SealReportPath = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-operator-ledger-bulletin-custody-register.json'
}
if (-not $OutputFile) {
  $OutputFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-custody-register-operator-governance-digest.json'
}
if (-not $OutputTextFile) {
  $OutputTextFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-custody-register-operator-governance-digest.txt'
}
...
$recommendedAction = 'publish_custody_register_operator_governance_digest'
$sealRan = $false

if (-not (Test-Path $SealScriptPath)) {
  Add-Finding -Severity 'warning' -Code 'custody_register_script_missing' -Message 'Operator ledger bulletin custody register script missing.' -Path $SealScriptPath
}
else {
  $cmd = @(
    '-NoLogo',
  ...
  if (-not $sealRan) {
    Add-Finding -Severity 'warning' -Code 'custody_register_execution_failed' -Message 'Custody register execution failed.' -Path $SealScriptPath
  }
}
...
if (Test-Path $SealReportPath) {
  try {
    ...
    if ($sealData.custody_register_state) {
      $operatorHandoffSealState = [string]$sealData.custody_register_state
    }
    if ($null -ne $sealData.custody_register_complete) {
      $operatorHandoffSealComplete = [bool]$sealData.custody_register_complete
    }
    ...
    if ($sealData.custody_register) {
      $operatorHandoffSeal = @($sealData.custody_register)
    }
  }
  catch {
  Add-Finding -Severity 'warning' -Code 'custody_register_report_parse_failed' -Message "Unable to parse custody register report JSON: $($_.Exception.Message)" -Path $SealReportPath
  }
}
else {
  Add-Finding -Severity 'warning' -Code 'custody_register_report_missing' -Message 'Custody register report missing after operator governance digest execution.' -Path $SealReportPath
}

$laneCustodyLedger = @(
  'operator_governance_digest_basis=custody_register',
  ...
  "custody_register_count=$(@($operatorHandoffSeal).Count)",
  "operator_acknowledged=$([bool]$OperatorAcknowledged)",
  'operator_governance_digest_note=Operator governance digest is recorded as local governance evidence only.'
)
...
if (-not $laneCustodyLedgerComplete) {
  ...
  $recommendedAction = 'repair_custody_register_and_rebuild_operator_governance_digest'
  Add-Finding -Severity 'warning' -Code 'operator_governance_digest_not_ready' -Message 'Custody register operator governance digest checks failed.' -Path $SealReportPath
}
...
$report = [ordered]@{
  ...
  operator_acknowledged = [bool]$OperatorAcknowledged
  operator_governance_digest_state = $laneCustodyLedgerState
  operator_governance_digest_complete = $laneCustodyLedgerComplete
  recommended_action = $recommendedAction
  seal_script_path = $SealScriptPath
  seal_report_path = $SealReportPath
  seal_ran = $sealRan
  custody_register_state = $operatorHandoffSealState
  custody_register_complete = $operatorHandoffSealComplete
  lane_profile_name = $laneProfileName
  profile_state = $profileState
  launch_command = $launchCommand
  ...
  custody_register = $operatorHandoffSeal
  operator_governance_digest = $laneCustodyLedger
  local_only_note = 'Custody register operator governance digest is governance-only and does not authorize runtime execution, provider expansion, placement, or project mutation.'
  findings = $findings
}
