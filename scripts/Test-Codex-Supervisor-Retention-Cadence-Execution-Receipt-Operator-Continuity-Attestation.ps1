...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-execution-receipt-operator-continuity-attestation-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Execution-Receipt-Operator-Continuity-Attestation.ps1'
  $reportPath = Join-Path $testQueue 'execution-receipt-operator-continuity-attestation.json'
  ...
  Assert-True ($readyReport.attestation_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.attestation_complete) 'acknowledged scenario attestation_complete should be true'
  ...
  Assert-True ($attentionReport.attestation_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.attestation_complete) 'non-acknowledged scenario attestation_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence execution receipt operator continuity attestation checks.' -ForegroundColor Green
}
finally {
