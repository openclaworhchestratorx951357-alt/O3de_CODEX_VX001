...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-startup-lane-bundle-continuity-checkpoint-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Startup-Lane-Bundle-Continuity-Checkpoint.ps1'
  $reportPath = Join-Path $testQueue 'startup-lane-bundle-continuity-checkpoint.json'
  ...
  Assert-True ($readyReport.continuity_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.continuity_ready) 'acknowledged scenario continuity_ready should be true'
  ...
  Assert-True ($attentionReport.continuity_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.continuity_ready) 'non-acknowledged scenario continuity_ready should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence startup lane bundle continuity checkpoint checks.' -ForegroundColor Green
}
finally {
