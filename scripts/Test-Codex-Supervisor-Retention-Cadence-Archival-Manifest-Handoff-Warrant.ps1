...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-archival-manifest-handoff-warrant-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Archival-Manifest-Handoff-Warrant.ps1'
  $reportPath = Join-Path $testQueue 'archival-manifest-handoff-warrant.json'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence archival manifest handoff warrant checks.' -ForegroundColor Green
}
finally {
