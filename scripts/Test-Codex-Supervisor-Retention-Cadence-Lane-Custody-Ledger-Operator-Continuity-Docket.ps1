...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-lane-custody-ledger-operator-continuity-docket-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Lane-Custody-Ledger-Operator-Continuity-Docket.ps1'
  $reportPath = Join-Path $testQueue 'lane-custody-ledger-operator-continuity-docket.json'
  ...
  Assert-True ($readyReport.operator_continuity_docket_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.operator_continuity_docket_complete) 'acknowledged scenario stewardship_receipt_complete should be true'
  ...
  Assert-True ($attentionReport.operator_continuity_docket_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.operator_continuity_docket_complete) 'non-acknowledged scenario stewardship_receipt_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence lane custody ledger operator continuity docket checks.' -ForegroundColor Green
}
finally {
