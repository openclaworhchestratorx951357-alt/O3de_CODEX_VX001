...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-readiness-ledger-handoff-seal-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Readiness-Ledger-Handoff-Seal.ps1'
  $reportPath = Join-Path $testQueue 'readiness-ledger-handoff-seal.json'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence readiness ledger handoff seal checks.' -ForegroundColor Green
}
finally {
