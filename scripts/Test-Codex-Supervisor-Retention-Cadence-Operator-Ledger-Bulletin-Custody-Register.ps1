...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-operator-ledger-bulletin-custody-register-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Ledger-Bulletin-Custody-Register.ps1'
  $reportPath = Join-Path $testQueue 'operator-ledger-bulletin-custody-register.json'
  ...
  Assert-True ($readyReport.custody_register_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.custody_register_complete) 'acknowledged scenario stewardship_receipt_complete should be true'
  ...
  Assert-True ($attentionReport.custody_register_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.custody_register_complete) 'non-acknowledged scenario stewardship_receipt_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence operator ledger bulletin custody register checks.' -ForegroundColor Green
}
finally {
