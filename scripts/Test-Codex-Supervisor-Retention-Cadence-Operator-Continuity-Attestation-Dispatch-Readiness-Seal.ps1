...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-operator-continuity-attestation-dispatch-readiness-seal-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Continuity-Attestation-Dispatch-Readiness-Seal.ps1'
  $reportPath = Join-Path $testQueue 'operator-continuity-attestation-dispatch-readiness-seal.json'
  ...
  Assert-True ($readyReport.seal_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.seal_complete) 'acknowledged scenario seal_complete should be true'
  ...
  Assert-True ($attentionReport.seal_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.seal_complete) 'non-acknowledged scenario seal_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence operator continuity attestation dispatch readiness seal checks.' -ForegroundColor Green
}
finally {
