...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-custody-register-operator-governance-digest-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Custody-Register-Operator-Governance-Digest.ps1'
  $reportPath = Join-Path $testQueue 'custody-register-operator-governance-digest.json'
  ...
  Assert-True ($readyReport.operator_governance_digest_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.operator_governance_digest_complete) 'acknowledged scenario stewardship_receipt_complete should be true'
  ...
  Assert-True ($attentionReport.operator_governance_digest_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.operator_governance_digest_complete) 'non-acknowledged scenario stewardship_receipt_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence custody register operator governance digest checks.' -ForegroundColor Green
}
finally {
