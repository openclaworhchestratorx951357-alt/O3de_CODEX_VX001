...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-continuity-register-handoff-index-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Continuity-Register-Handoff-Index.ps1'
  $reportPath = Join-Path $testQueue 'continuity-register-handoff-index.json'
  ...
  Assert-True ($readyReport.index_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.index_complete) 'acknowledged scenario index_complete should be true'
  ...
  Assert-True ($attentionReport.index_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.index_complete) 'non-acknowledged scenario index_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence continuity register handoff index checks.' -ForegroundColor Green
}
finally {
