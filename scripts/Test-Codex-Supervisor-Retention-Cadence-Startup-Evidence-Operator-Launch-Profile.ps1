...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-startup-evidence-operator-launch-profile-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Startup-Evidence-Operator-Launch-Profile.ps1'
  $reportPath = Join-Path $testQueue 'startup-operator-launch-profile.json'
  ...
  Assert-True ($readyReport.profile_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.operator_acknowledged) 'acknowledged scenario should record operator acknowledgment'
  Assert-True ([bool]$readyReport.startup_gate_ready) 'acknowledged scenario startup_gate_ready should be true'
  ...
  Assert-True ($attentionReport.profile_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence startup evidence operator launch profile checks.' -ForegroundColor Green
}
finally {
