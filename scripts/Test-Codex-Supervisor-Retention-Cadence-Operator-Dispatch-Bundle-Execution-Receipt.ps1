...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-operator-dispatch-bundle-execution-receipt-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Dispatch-Bundle-Execution-Receipt.ps1'
  $reportPath = Join-Path $testQueue 'operator-dispatch-bundle-execution-receipt.json'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence operator dispatch bundle execution receipt checks.' -ForegroundColor Green
}
finally {
