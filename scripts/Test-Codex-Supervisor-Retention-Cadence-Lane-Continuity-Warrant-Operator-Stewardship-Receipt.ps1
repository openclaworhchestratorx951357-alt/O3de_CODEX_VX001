...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-lane-continuity-warrant-operator-stewardship-receipt-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Lane-Continuity-Warrant-Operator-Stewardship-Receipt.ps1'
  $reportPath = Join-Path $testQueue 'lane-continuity-warrant-operator-stewardship-receipt.json'
  ...
  Assert-True ($readyReport.stewardship_receipt_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.stewardship_receipt_complete) 'acknowledged scenario stewardship_receipt_complete should be true'
  ...
  Assert-True ($attentionReport.stewardship_receipt_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.stewardship_receipt_complete) 'non-acknowledged scenario stewardship_receipt_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence lane continuity warrant operator stewardship receipt checks.' -ForegroundColor Green
}
finally {
