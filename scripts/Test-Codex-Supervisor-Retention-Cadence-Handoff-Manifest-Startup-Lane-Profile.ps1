...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-handoff-manifest-startup-lane-profile-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Handoff-Manifest-Startup-Lane-Profile.ps1'
  $reportPath = Join-Path $testQueue 'handoff-manifest-startup-lane-profile.json'
  ...
  Assert-True ($readyReport.lane_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.lane_ready) 'acknowledged scenario lane_ready should be true'
  ...
  Assert-True ($attentionReport.lane_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.lane_ready) 'non-acknowledged scenario lane_ready should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence handoff manifest startup lane profile checks.' -ForegroundColor Green
}
finally {
