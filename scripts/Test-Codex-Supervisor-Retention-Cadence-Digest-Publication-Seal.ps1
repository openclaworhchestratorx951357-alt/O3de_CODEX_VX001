...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-digest-publication-seal-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Digest-Publication-Seal.ps1'
  $reportPath = Join-Path $testQueue 'digest-publication-seal.json'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence digest publication seal checks.' -ForegroundColor Green
}
finally {
