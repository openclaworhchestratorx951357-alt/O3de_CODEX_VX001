...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-closeout-ledger-archival-manifest-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Closeout-Ledger-Archival-Manifest.ps1'
  $reportPath = Join-Path $testQueue 'closeout-ledger-archival-manifest.json'
  ...
  Assert-True ([bool]$readyReport.manifest_complete) 'acknowledged scenario manifest_complete should be true'
  ...
  Assert-True (-not [bool]$attentionReport.manifest_complete) 'non-acknowledged scenario manifest_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence closeout ledger archival manifest checks.' -ForegroundColor Green
}
finally {
