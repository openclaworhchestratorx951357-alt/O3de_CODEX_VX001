...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-dispatch-readiness-seal-operator-runbook-attestation-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Dispatch-Readiness-Seal-Operator-Runbook-Attestation.ps1'
  $reportPath = Join-Path $testQueue 'dispatch-readiness-seal-operator-runbook-attestation.json'
  ...
  Assert-True ($readyReport.runbook_attestation_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.runbook_attestation_complete) 'acknowledged scenario runbook_attestation_complete should be true'
  ...
  Assert-True ($attentionReport.runbook_attestation_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.runbook_attestation_complete) 'non-acknowledged scenario runbook_attestation_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence dispatch readiness seal operator runbook attestation checks.' -ForegroundColor Green
}
finally {
