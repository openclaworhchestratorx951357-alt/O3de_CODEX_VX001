...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-execution-checklist-evidence-handoff-digest-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Execution-Checklist-Evidence-Handoff-Digest.ps1'
  $reportPath = Join-Path $testQueue 'execution-checklist-evidence-handoff-digest.json'
  ...
  Assert-True ($readyReport.digest_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.digest_complete) 'acknowledged scenario digest_complete should be true'
  ...
  Assert-True ($attentionReport.digest_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.digest_complete) 'non-acknowledged scenario digest_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence execution checklist evidence handoff digest checks.' -ForegroundColor Green
}
finally {
