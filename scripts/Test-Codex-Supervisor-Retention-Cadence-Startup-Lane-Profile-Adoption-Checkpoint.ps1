...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-startup-lane-profile-adoption-checkpoint-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Startup-Lane-Profile-Adoption-Checkpoint.ps1'
  $reportPath = Join-Path $testQueue 'startup-lane-profile-adoption-checkpoint.json'
  ...
  Assert-True ($readyReport.checkpoint_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.adoption_ready) 'acknowledged scenario adoption_ready should be true'
  ...
  Assert-True ($attentionReport.checkpoint_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.adoption_ready) 'non-acknowledged scenario adoption_ready should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence startup lane profile adoption checkpoint checks.' -ForegroundColor Green
}
finally {
