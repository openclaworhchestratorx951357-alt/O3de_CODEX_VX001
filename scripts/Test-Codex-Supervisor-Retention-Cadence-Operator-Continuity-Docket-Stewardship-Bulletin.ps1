...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-operator-continuity-docket-stewardship-bulletin-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Operator-Continuity-Docket-Stewardship-Bulletin.ps1'
  $reportPath = Join-Path $testQueue 'operator-continuity-docket-stewardship-bulletin.json'
  ...
  Assert-True ($readyReport.stewardship_bulletin_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.stewardship_bulletin_complete) 'acknowledged scenario stewardship_receipt_complete should be true'
  ...
  Assert-True ($attentionReport.stewardship_bulletin_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.stewardship_bulletin_complete) 'non-acknowledged scenario stewardship_receipt_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence operator continuity docket stewardship bulletin checks.' -ForegroundColor Green
}
finally {
