param(
  ...
  [string]$LedgerName = 'operator_ledger_bulletin_custody_register',
...
if (-not $SealScriptPath) {
  $SealScriptPath = Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Custody-Gazette-Operator-Ledger-Bulletin.ps1'
}
if (-not $SealReportPath) {
  $SealReportPath = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-custody-gazette-operator-ledger-bulletin.json'
}
if (-not $OutputFile) {
  $OutputFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-operator-ledger-bulletin-custody-register.json'
}
if (-not $OutputTextFile) {
  $OutputTextFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-operator-ledger-bulletin-custody-register.txt'
}
...
$recommendedAction = 'publish_operator_ledger_bulletin_custody_register'
$sealRan = $false

if (-not (Test-Path $SealScriptPath)) {
  Add-Finding -Severity 'warning' -Code 'operator_ledger_bulletin_script_missing' -Message 'Custody gazette operator ledger bulletin script missing.' -Path $SealScriptPath
}
else {
  $cmd = @(
    '-NoLogo',
  ...
  if (-not $sealRan) {
    Add-Finding -Severity 'warning' -Code 'operator_ledger_bulletin_execution_failed' -Message 'Operator ledger bulletin execution failed.' -Path $SealScriptPath
  }
}
...
if (Test-Path $SealReportPath) {
  try {
    ...
    if ($sealData.operator_ledger_bulletin_state) {
      $operatorHandoffSealState = [string]$sealData.operator_ledger_bulletin_state
    }
    if ($null -ne $sealData.operator_ledger_bulletin_complete) {
      $operatorHandoffSealComplete = [bool]$sealData.operator_ledger_bulletin_complete
    }
    ...
    if ($sealData.operator_ledger_bulletin) {
      $operatorHandoffSeal = @($sealData.operator_ledger_bulletin)
    }
  }
  catch {
  Add-Finding -Severity 'warning' -Code 'operator_ledger_bulletin_report_parse_failed' -Message "Unable to parse operator ledger bulletin report JSON: $($_.Exception.Message)" -Path $SealReportPath
  }
}
else {
  Add-Finding -Severity 'warning' -Code 'operator_ledger_bulletin_report_missing' -Message 'Operator ledger bulletin report missing after custody register execution.' -Path $SealReportPath
}

$laneCustodyLedger = @(
  'custody_register_basis=operator_ledger_bulletin',
  ...
  "operator_ledger_bulletin_count=$(@($operatorHandoffSeal).Count)",
  "operator_acknowledged=$([bool]$OperatorAcknowledged)",
  'custody_register_note=Custody register is recorded as local governance evidence only.'
)
...
if (-not $laneCustodyLedgerComplete) {
  ...
  $recommendedAction = 'repair_operator_ledger_bulletin_and_rebuild_custody_register'
  Add-Finding -Severity 'warning' -Code 'custody_register_not_ready' -Message 'Operator ledger bulletin custody register checks failed.' -Path $SealReportPath
}
...
$report = [ordered]@{
  ...
  operator_acknowledged = [bool]$OperatorAcknowledged
  custody_register_state = $laneCustodyLedgerState
  custody_register_complete = $laneCustodyLedgerComplete
  recommended_action = $recommendedAction
  seal_script_path = $SealScriptPath
  seal_report_path = $SealReportPath
  seal_ran = $sealRan
  operator_ledger_bulletin_state = $operatorHandoffSealState
  operator_ledger_bulletin_complete = $operatorHandoffSealComplete
  lane_profile_name = $laneProfileName
  profile_state = $profileState
  launch_command = $launchCommand
  ...
  operator_ledger_bulletin = $operatorHandoffSeal
  custody_register = $laneCustodyLedger
  local_only_note = 'Operator ledger bulletin custody register is governance-only and does not authorize runtime execution, provider expansion, placement, or project mutation.'
  findings = $findings
}
