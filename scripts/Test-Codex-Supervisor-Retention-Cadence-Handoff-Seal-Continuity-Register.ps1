...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-handoff-seal-continuity-register-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Handoff-Seal-Continuity-Register.ps1'
  $reportPath = Join-Path $testQueue 'handoff-seal-continuity-register.json'
  ...
  Assert-True ($readyReport.register_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.register_complete) 'acknowledged scenario register_complete should be true'
  ...
  Assert-True ($attentionReport.register_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.register_complete) 'non-acknowledged scenario register_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence handoff seal continuity register checks.' -ForegroundColor Green
}
finally {
