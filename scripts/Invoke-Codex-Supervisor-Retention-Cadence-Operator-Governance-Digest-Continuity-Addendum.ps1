param(
  ...
  [string]$LedgerName = 'operator_governance_digest_continuity_addendum',
...
if (-not $SealScriptPath) {
  $SealScriptPath = Join-Path $RepoRoot 'scripts\Invoke-Codex-Supervisor-Retention-Cadence-Custody-Register-Operator-Governance-Digest.ps1'
}
if (-not $SealReportPath) {
  $SealReportPath = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-custody-register-operator-governance-digest.json'
}
if (-not $OutputFile) {
  $OutputFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-operator-governance-digest-continuity-addendum.json'
}
if (-not $OutputTextFile) {
  $OutputTextFile = Join-Path $RepoRoot 'continue-queue\codex-supervisor-retention-cadence-operator-governance-digest-continuity-addendum.txt'
}
...
$recommendedAction = 'publish_operator_governance_digest_continuity_addendum'
$sealRan = $false

if (-not (Test-Path $SealScriptPath)) {
  Add-Finding -Severity 'warning' -Code 'operator_governance_digest_script_missing' -Message 'Custody register operator governance digest script missing.' -Path $SealScriptPath
}
else {
  $cmd = @(
    '-NoLogo',
  ...
  if (-not $sealRan) {
    Add-Finding -Severity 'warning' -Code 'operator_governance_digest_execution_failed' -Message 'Operator governance digest execution failed.' -Path $SealScriptPath
  }
}
...
if (Test-Path $SealReportPath) {
  try {
    ...
    if ($sealData.operator_governance_digest_state) {
      $operatorHandoffSealState = [string]$sealData.operator_governance_digest_state
    }
    if ($null -ne $sealData.operator_governance_digest_complete) {
      $operatorHandoffSealComplete = [bool]$sealData.operator_governance_digest_complete
    }
    ...
    if ($sealData.operator_governance_digest) {
      $operatorHandoffSeal = @($sealData.operator_governance_digest)
    }
  }
  catch {
  Add-Finding -Severity 'warning' -Code 'operator_governance_digest_report_parse_failed' -Message "Unable to parse operator governance digest report JSON: $($_.Exception.Message)" -Path $SealReportPath
  }
}
else {
  Add-Finding -Severity 'warning' -Code 'operator_governance_digest_report_missing' -Message 'Operator governance digest report missing after continuity addendum execution.' -Path $SealReportPath
}

$laneCustodyLedger = @(
  'continuity_addendum_basis=operator_governance_digest',
  ...
  "operator_governance_digest_count=$(@($operatorHandoffSeal).Count)",
  "operator_acknowledged=$([bool]$OperatorAcknowledged)",
  'continuity_addendum_note=Continuity addendum is recorded as local governance evidence only.'
)
...
if (-not $laneCustodyLedgerComplete) {
  ...
  $recommendedAction = 'repair_operator_governance_digest_and_rebuild_continuity_addendum'
  Add-Finding -Severity 'warning' -Code 'operator_governance_digest_not_ready' -Message 'Operator governance digest continuity addendum checks failed.' -Path $SealReportPath
}
...
$report = [ordered]@{
  ...
  operator_acknowledged = [bool]$OperatorAcknowledged
  operator_governance_digest_continuity_addendum_state = $laneCustodyLedgerState
  operator_governance_digest_continuity_addendum_complete = $laneCustodyLedgerComplete
  recommended_action = $recommendedAction
  seal_script_path = $SealScriptPath
  seal_report_path = $SealReportPath
  seal_ran = $sealRan
  operator_governance_digest_state = $operatorHandoffSealState
  operator_governance_digest_complete = $operatorHandoffSealComplete
  lane_profile_name = $laneProfileName
  profile_state = $profileState
  launch_command = $launchCommand
  ...
  operator_governance_digest = $operatorHandoffSeal
  operator_governance_digest_continuity_addendum = $laneCustodyLedger
  local_only_note = 'Operator governance digest continuity addendum is governance-only and does not authorize runtime execution, provider expansion, placement, or project mutation.'
  findings = $findings
}
