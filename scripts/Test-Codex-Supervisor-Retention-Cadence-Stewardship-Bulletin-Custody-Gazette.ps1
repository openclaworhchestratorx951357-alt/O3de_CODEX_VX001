...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-stewardship-bulletin-custody-gazette-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Stewardship-Bulletin-Custody-Gazette.ps1'
  $reportPath = Join-Path $testQueue 'stewardship-bulletin-custody-gazette.json'
  ...
  Assert-True ($readyReport.custody_gazette_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.custody_gazette_complete) 'acknowledged scenario stewardship_receipt_complete should be true'
  ...
  Assert-True ($attentionReport.custody_gazette_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.custody_gazette_complete) 'non-acknowledged scenario stewardship_receipt_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence stewardship bulletin custody gazette checks.' -ForegroundColor Green
}
finally {
