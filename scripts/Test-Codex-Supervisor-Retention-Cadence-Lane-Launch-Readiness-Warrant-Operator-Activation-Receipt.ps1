...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-supervisor-retention-cadence-lane-launch-readiness-warrant-operator-activation-receipt-test-" + [Guid]::NewGuid().ToString())
...
try {
  ...
  $invokePath = Join-Path $testScripts 'Invoke-Codex-Supervisor-Retention-Cadence-Lane-Launch-Readiness-Warrant-Operator-Activation-Receipt.ps1'
  $reportPath = Join-Path $testQueue 'lane-launch-readiness-warrant-operator-activation-receipt.json'
  ...
  Assert-True ($readyReport.receipt_state -eq 'ready') 'acknowledged scenario should be ready'
  Assert-True ([bool]$readyReport.receipt_complete) 'acknowledged scenario receipt_complete should be true'
  ...
  Assert-True ($attentionReport.receipt_state -eq 'attention_required') 'non-acknowledged scenario should be attention_required'
  Assert-True (-not [bool]$attentionReport.receipt_complete) 'non-acknowledged scenario receipt_complete should be false'
  ...
  Write-Host 'PASS: Flow Trigger Suite retention cadence lane launch readiness warrant operator activation receipt checks.' -ForegroundColor Green
}
finally {
